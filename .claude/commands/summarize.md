...
- **Existing output**: if `outputs/<slug>-summary.md` already exists, ask the
  user (AskUserQuestion) whether to overwrite or use a different slug. Do not proceed until
  confirmed.
...
One Agent call per chunk, batched in parallel (one message, multiple Agent
calls), each with `subagent_type: "researcher"` and this prompt:

> Read ONLY `outputs/.notes/<slug>-chunk-NNN.txt`. Extract: (1) definitions
> and theorem statements with their numbers, (2) key claims and proof ideas,
> (3) cited references. Do NOT use web search or fetch external URLs — this
> is single-source summarization. If a statement or proof appears to start or
> end mid-way at the file boundary, mark it BOUNDARY PARTIAL. Write to
> `outputs/.notes/<slug>-summary-chunk-NNN.md`.

A failed chunk does not abort the others; missing chunk summaries surface in
