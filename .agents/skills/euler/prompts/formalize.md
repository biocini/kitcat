...
Formalize the following target into the proof library: $@
...
Before anything else, locate the toolchain block: `TOOLCHAIN.md` at the
project root, or a `## Toolchain` section in the project's `AGENTS.md`.
...
gathering evidence or writing code. Summarize the plan briefly and ask:

`Proceed with this formalization plan? Reply "yes" to continue, or tell me what to change.`
...
- Keep subagent call payloads small and valid; put multi-paragraph
  instructions in the on-disk brief, not in the call.
- Use only supported subagent keys. Set `failFast: false` for parallel
  dispatches.
- Do not name exact tool commands in subagent tasks unless those tools are
...
yourself; otherwise dispatch the `verifier` agent:

```json
{
  "agent": "verifier",
  "task": "Verify outputs/.drafts/<slug>-report-draft.md against the research files and the library. Source layer: anchor every informal claim. Kernel layer: run the toolchain block's check command, audit obligation and unsafe markers in the delivered files, and transcription-check every quoted statement. Write the complete verified report to outputs/.drafts/<slug>-verified.md.",
  "output": "outputs/.drafts/<slug>-verified.md"
}
```
...
`reviewer` agent against the verified report plus the delivered code:

```json
{
  "agent": "reviewer",
  "task": "Review the formalization described in outputs/.drafts/<slug>-verified.md and delivered in the library files it lists. Prioritize statement fidelity against the informal source, definition gaming, smuggled hypotheses, and undisclosed obligations. This is a verification pass, not a style review.",
  "output": "outputs/.drafts/<slug>-review.md"
}
```
