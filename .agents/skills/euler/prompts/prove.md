...
Start a bounded proof loop for: $@
...
ask the user if they want to resume or start fresh. If resuming, also verify
...
Resolve the toolchain block first (`TOOLCHAIN.md` at project root or a
`## Toolchain` section in the project `AGENTS.md`). The check command comes
...
Then ask the user where to run:
...
loop-state files: fixed names, one active loop per project (see AGENTS.md);
