...
recommendations and `docs/gloss.md` entries are proposals, recorded
in the artifact and sidecar — never executed as a side effect; a
candidate `resources/` entry is likewise proposal-only for any
source no load-bearing claim rests on. The one carve-out is
ingest-on-firsthand-need: a citation the matrix relies on that rests
on an unvendored source triggers the Cite step's `ingest` dispatch,
which vendors a directed PROVISIONAL entry per the contract.
