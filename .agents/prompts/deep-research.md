...
(mechanization spikes, `docs/gloss.md` entries) are proposals
recorded in the brief and sidecar, never executed as a side effect;
a `resources/` entry is likewise proposal-only for any source no
load-bearing claim rests on. The one carve-out is
ingest-on-firsthand-need: a citation the brief relies on that rests
