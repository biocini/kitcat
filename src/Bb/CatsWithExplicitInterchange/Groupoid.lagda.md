...
The path groupoid of a type, as a Bb.CatsWithExplicitInterchange.Type
category.
...
The `category` record is from `Bb.CatsWithExplicitInterchange.Type`; the
ternary embedding and its equivalence are from `Core.Groupoid.Virtual`.
The instance bridges the two, so it lives in its own module and keeps
`Bb.CatsWithExplicitInterchange.Type` free of the groupoid dependency.
...
module Bb.CatsWithExplicitInterchange.Groupoid where
