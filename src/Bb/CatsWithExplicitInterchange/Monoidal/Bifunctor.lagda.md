...
composition, the displaced unit and composite comparisons, the displaced
representability and witness calculi, and naturality of the associator
and unitors. The archived single-field form is
`Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Bifunctor`; the split
here follows the record: every cell consuming an interchange lives in
`over-interchange-bifunctor`, developed once over an arbitrary pointwise
pair and instantiated at either field, and every cell that never touches
one ports unchanged.
...
module Bb.CatsWithExplicitInterchange.Monoidal.Bifunctor where
...
certifies the contraction is the same through `ι⁻`: everything below is
insensitive to the choice. With the frame endpoints fixed, `⊗₁-emb` is
an ordinary map between fixed types, and its plain fibers support the
`Bb.CatsWithExplicitInterchange.Base` calculus by transcription.
