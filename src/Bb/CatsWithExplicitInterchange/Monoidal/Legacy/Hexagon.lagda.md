...
The irreducible coherence layer for the braided monoidal structure. The
associator is free and the braiding is free
(`Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Braid`), but the
hexagon is not: the two braid traversals of a triple composite —
braiding past the composite in one step, and braiding past each factor
separately — are two genuinely distinct paths between the same ternary
operations, and their identification is an axiom. Strict associativity
of the ternary orders collapses the old formulation's re-nesting
entirely: each field is one 2-path between a braid and a composite of
whiskered braids, at witness arguments, with its morphism-grade
displacement over it.
...
module Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Hexagon where
...
```agda
  ...
  braid●₀-nrm U V m = braid●₀ (nrm-slide₀ U m) (nrm-slide₀ V m)
  ...
  braid●₁-nrm Û V̂ m = braid●₁ (nrm-slide₁ Û m) (nrm-slide₁ V̂ m)
```
...
```agda
    ...
    step-r₁ = ap-comp fst μ rt₁ ∙ Path.unitl (ap fst rt₁)

    step-r₂
      : ap fst rt₁ ≡ ap (_⊗₀ z) (⊗₀-braid x y) ∙ sym (⊗₀-assoc y x z) ∙ ap (y ⊗₀_) (⊗₀-braid x z)
    step-r₂ =
    ...
    ⊗₀-hexagon-r
      ...
      ≡ ap (_⊗₀ z) (⊗₀-braid x y)
        ∙ sym (⊗₀-assoc y x z)
        ∙ ap (y ⊗₀_) (⊗₀-braid x z)
    ⊗₀-hexagon-r = sym sl ∙ ap (ap fst) fiber-hexagon ∙ sr
```
...
```agda
    step-l₁
      : ap fst ℓc ≡ ⊗₀-assoc x y z ∙ braid●₀ (⊗₀-nrm x ●₀ ⊗₀-nrm y) (⊗₀-nrm z) ∙ ⊗₀-assoc z x y
    step-l₁ =
        ap-comp fst ℓ-assoc₁ ℓt
    ...
    step-r₂ : ap fst rt₁ ≡ ap (x ⊗₀_) (⊗₀-braid y z) ∙ ⊗₀-assoc x z y ∙ ap (_⊗₀ y) (⊗₀-braid x z)
    step-r₂ =
        ap-comp fst r-braid₁ rt₂
    ...
    ⊗₀-hexagon-l
      ...
      ≡ ap (x ⊗₀_) (⊗₀-braid y z)
        ∙ ⊗₀-assoc x z y
        ∙ ap (_⊗₀ y) (⊗₀-braid x z)
    ⊗₀-hexagon-l = sym sl ∙ ap (ap fst) fiber-hexagon ∙ sr
```
...
```agda
      ...
      inner̂₂ : PathP (λ m → FamII (uρ m) (uρ' m)) T₀ T₂
      inner̂₂ =
        comp-pathp₂ FamII
      ...
      inner̂₁ : PathP (λ m → FamI (vα m) (vα' m)) S₀ S₂
      inner̂₁ =
        comp-pathp₂ FamI
