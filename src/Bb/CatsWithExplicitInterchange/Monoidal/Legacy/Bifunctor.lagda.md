...
`theory₁`: the morphism-level derived theory of a
`Bb.CatsWithExplicitInterchange.Monoidal.Legacy` category —
functoriality of the derived 2-cell tensor `_⊗₁_` in vertical
composition, the displaced unit and composite comparisons, and
...
module Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Bifunctor where
...
exactly as `Bb.CatsWithExplicitInterchange.Base`'s `cast-path` pair.
...
With the frame endpoints fixed, `⊗₁-emb` is an ordinary map between
fixed types, and its plain fibers support the
`Bb.CatsWithExplicitInterchange.Base` representability calculus — this
part is transcription, not displacement. The one genuinely level-1 input
is the image-fiber contraction: the `q-char` fiber at the identity slot
is straightened to the plain image fiber along `⊗₁-idn-▴`, one `subst`
in a line of Σ-of-PathP types. The straightening square is the only
2-cell; its content is the computation of `ap ⊗₀-emb (⊗₀-unitl x)` out
...
From here the calculus is verbatim `Bb.CatsWithExplicitInterchange.Base`
under the dictionary `emb ↦ ⊗₁-emb`, `composite ↦ ⊗₁-composite`:
uniqueness of representing morphisms, its left-cancellability on paths,
and the total-space equivalence.
