...
Invertibility is free: the object braiding is a path in `ob`, so its
symmetry is its inverse — there is no separate axiom, unlike the
classical definition where invertibility is an imposed condition on a
natural transformation. The hexagon coherences are *not* free; they live
in `Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Hexagon`.

```agda
{-# OPTIONS --safe --erased-cubical --no-guardedness #-}

module Bb.CatsWithExplicitInterchange.Monoidal.Legacy.Braid where
