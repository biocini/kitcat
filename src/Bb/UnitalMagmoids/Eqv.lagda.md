...
import Bb.UnitalMagmoids.Neutral as N
import Bb.UnitalMagmoids.Unit as U
import Bb.UnitalMagmoids.Iso as I

module Bb.UnitalMagmoids.Eqv (M : magmoids) (u : ∀ x → N.unital M x) where
...
open import Bb.UnitalMagmoids.Base M hiding (assoc)
open N M
...
module associator (assoc : associativity) where
  ...
  module _ {x y} ((f , s , r) : x ≃ y) where
    private
      s≡r : s .fst ≡ r .fst
      s≡r = pcom (unitl (s .fst)) (sym (r .snd) ▹ (s .fst))
              (pcom (assoc (r .fst) f (s .fst)) (r .fst ◃ (s .snd)) (unitr (r .fst)))
