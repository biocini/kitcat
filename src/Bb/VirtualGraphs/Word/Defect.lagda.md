...
depends on one flanking edge alone. `w⁺ (rise f)` flanks the leading
side through the positive cut, where `rise f` is the value of the edge
at zero. `w⁻ (zrunW h)` flanks the trailing side through the negative
cut, where `zrunW h` is the length of the zero plateau of `h`. The
...
The defect is not a framing constant. `no-uniform⁺` and `no-uniform⁻`
show that no single word fills either surviving placement for all
triples. Over `L = (f ⨾⁺ g) ⨾⁻ h` and `R = f ⨾⁺ (g ⨾⁻ h)`, the
whole-word placements put the word on one side of one hand, and the
seam placements move it inside one bracketing; the fourteen remaining
ones each fail at a concrete triple, for every word at once.

Both corrections carry only the winding grade of their unit, so the
collapse onto the grade group erases them: the two bracketings of
`associates` always agree in winding grade.

```agda
{-# OPTIONS --safe --erased-cubical --no-guardedness #-}

module Bb.VirtualGraphs.Word.Defect where
...
plateau: under weak monotonicity the edge is zero strictly below it and
positive from it on.
...
The leading correction: for every triple, the right bracketing is the
left one cut against `w⁺ (rise f)` on the outside of the positive hand.
Weak monotonicity pins the corrected values, and the zero case is the
axiom value itself.
...
The trailing correction: for every triple, the left bracketing is the
right one cut against `w⁻ (zrunW h)` on the inside of the negative hand.
Below the plateau both sides vanish, and from the plateau on the guard
steps aside.
...
The leading correction is a unit exactly when `rise f` is zero, and that
is exactly thunkability. Dually for the trailing correction and
linearity. So the defect words measure the two closures.
...
Each remaining placement fails at a concrete triple, for every `w`. The
whole-word placements: `A1` is `R ≡ L ⨾⁺ w`, `A3` is `R ≡ L ⨾⁻ w`, `A4`
is `R ≡ w ⨾⁻ L`, `A5` is `L ≡ R ⨾⁺ w`, `A6` is `L ≡ w ⨾⁺ R`, `A8` is
`L ≡ w ⨾⁻ R`. The refuting triples are `(τ̂ , τ̂ , ε̂)`, where the left
bracketing must supply the value one and cannot, and `(τ̂ , ε̂ , ε̂)`,
where a flank forces zero against one or pins two values of `w` at once.
...
The seam placements, at the same two triples. `S1` through `S4` correct
inside the right bracketing toward the left, and the head of the leading
edge forces a positive value where the left bracketing is zero. `S5`,
`S6`, `S8` correct inside the left bracketing toward the right, and the
guarding cut forces zero where the right bracketing is one. `S7`
reroutes the trailing edge and misses the value one.
