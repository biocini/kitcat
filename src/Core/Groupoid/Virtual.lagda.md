...
module Core.Groupoid.Virtual where
...
open import Core.Equiv.Base
  using (is-equiv; iso→equiv; eqv-fibers; is-contr-equiv; _≃_)
...
## Binary composition from contractible path space

`SinglP-contr x` says `Σ y, PathP A x y` is contractible.
`repr.emb-equiv` says `PathP A x y ≃ (∀ w → w ≡ x → ∀ z → y ≡ z → PathP A w z)`.
Transporting across the equivalence:

`is-contr (Σ y, ∀ w → w ≡ x → ∀ z → y ≡ z → PathP A w z)`

The center is `(coe x, repr.emb (coe-filler x))` — the transport
of `x` paired with the ternary action of its canonical filler. At
constant type family, this reduces to `(x, repr.emb refl)`: the
identity with canonical binary composition as its ternary action.

`Cat.Virtual`'s `compose-contr` generalizes this from h-level 0
(contractible = equivalence) to h-level 1 (propositional = embedding).

```agda

module _ {A : I → Type u} {x : A i0} where
  repr-singl-contr
    : is-contr
        (Σ y ∶ A i1
  ...
  repr-singl-contr =
    is-contr-equiv
      (esym (Σ-equiv-snd
        λ _ → repr.emb , repr.emb-equiv))
      (SinglP-contr x)

module _ {A : Type u} {x : A} where
  emb-id-eval : repr.emb {A = λ _ → A} (refl {x = x}) x refl
    ≡ λ v (b : x ≡ v) → b
  emb-id-eval i v b = Path.unitl b i
