...
`theory₁`: the morphism-level derived theory of a `Cat.Depreciated.Monoidal.Legacy`
...
module Cat.Depreciated.Monoidal.Legacy.Bifunctor where
...
`Cat.Depreciated.Base` representability calculus — this part is
...
From here the calculus is verbatim `Cat.Depreciated.Base` under the
