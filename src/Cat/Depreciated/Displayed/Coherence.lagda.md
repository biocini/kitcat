...
The displayed pentagon: `Cat.Depreciated.Coherence`'s `pentagon` tree, displaced
...
This is `Cat.Depreciated.Monoidal.Coherence`'s `pentagon₁` with the two-sided
`⊗₁-wit` calculus replaced by the one-sided displayed witness
calculus of `Cat.Depreciated.Displayed.Base`: `comp-pathp₂` becomes
...
`Cat.Depreciated.Coherence`'s σ-square tree — `triangle₁` under the same
...
module Cat.Depreciated.Displayed.Coherence where
...
The pentagon over `Cat.Depreciated.Coherence`'s `pentagon` itself: a square
...
The triangle over `Cat.Depreciated.Coherence`'s `triangle`: a square of
