...
`Cat.Depreciated.Base.theory` clause by clause. Unlike the monoidal `theory₀` —
a transcription whose delta was pure erasure — this is a genuinely
displaced development: each `∙` in a `Cat.Depreciated.Base` proof becomes a
`comp-pathp₁` over the composed base path, each `subst` a transport
along a line of displayed types. The proof shapes follow `Cat.Depreciated.Base`
line by line, and the monoidal `theory₁` is the worked one-object
special case of exactly this glue.
...
module Cat.Depreciated.Displayed.Base where
...
Each link of `Cat.Depreciated.Base`'s unit chain has a displayed image over the
same base path, glued link by link by `comp-pathp₁` at the family
`hom[_]`; the second factor of each link is the pointwise action of
a displayed operator on `unitᴰ`.
