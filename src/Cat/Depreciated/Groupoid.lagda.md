...
The path groupoid of a type, as a Cat.Depreciated.Type category.
...
The `category` record is from `Cat.Depreciated.Type`; the ternary embedding
and its equivalence are from `Core.Groupoid.Virtual`. The
instance bridges the two, so it lives in its own module and keeps
`Cat.Depreciated.Type` free of the groupoid dependency.
...
module Cat.Depreciated.Groupoid where
