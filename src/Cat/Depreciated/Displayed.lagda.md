...
A displayed category over a `Cat.Depreciated.Type` category: the representable
...
over `unit f`. The displayed axioms therefore open `Cat.Depreciated.Base.theory`,
...
theorems of `Cat.Depreciated.Displayed.Base.theoryᴰ`. This is the contrast with
...
instance — and `Cat.Depreciated.Type` itself is the instance over the point. The one
...
module Cat.Depreciated.Displayed where
...
spine center's projections, so `Cat.Depreciated.Base.theory` is in scope — the
