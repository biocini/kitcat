...
Classified virtual categories. Composition is gated by a
propositional classifier on composable pairs. The composition
target is the noy-side `emb f w a v (noy g v b)`, same as
Cat.Virtual. Interchange equates the noy and yon views for
classified pairs. The combinator `⟨ f , g , c ⟩` is derived
as the noy-side target.

Cat.Virtual is the special case where the classifier is
contractible.

```agda
{-# OPTIONS --safe --erased-cubical --no-guardedness --no-sized-types #-}

module Cat.Classified where
...
```agda
record classified o h p : Type₊ (o ⊔ h ⊔ p) where
  no-eta-equality
  field
    ob  : Type o
    hom : ob → ob → Type h
  ...
  field
    ...
    classifier-prop : ∀ {x y z} {f : hom x y} {g : hom y z}
    ...
    classifier-idn-l : ∀ {x y} (f : hom x y) → classifier idn f
    classifier-idn-r : ∀ {x y} (f : hom x y) → classifier f idn
  ...
  emb-composite : ∀ {x y z} (f : hom x y) (g : hom y z)
    → (c : classifier f g)
    → ∀ w (a : hom w x) v (b : hom z v)
    → emb (comp f g c) w a v b
    ≡ emb f w a v (noy g v b)
  emb-composite f g c = compose-classified f g c .center .snd
...
```agda
module Classified {o h p} (C : classified o h p) where
  open classified C public
  ...
  private
    ...
    noy-composite-idn {x} b =
      emb-composite idn idn idn-class x idn _ b
    ...
    yon-composite-idn {x} {w} a =
      emb-composite idn idn idn-class w a x idn
      ∙ interchange-classified idn idn idn-class w a x idn

    comp-eq-idn : ∀ {x} → comp (idn {x}) idn idn-class ≡ yon idn x idn
    comp-eq-idn =
  ...
  combinator-idn-l f w a v b =
    interchange-classified idn f (classifier-idn-l f) w a v b
...
fiber with `(comp f g c, emb-composite f g c)` as center.
`emb-ind` eliminates any `(s, q)` in the fiber back to the
canonical center.
...
```agda
  ...
  composable-contr f g c .center =
    comp f g c , emb-composite f g c
  composable-contr f g c .paths (s , p) =
    compose-classified f g c .paths (s , p)
  ...
  emb-ind
    ...
    → (P : (s : hom x z)
         → (∀ w (a : hom w x) v (b : hom z v)
             ...
             ≡ emb f w a v (noy g v b))
         → Type u)
    → P (comp f g c) (emb-composite f g c)
    → ∀ s q → P s q
...
`emb-composite-ext` lifts the pointwise composite
characterization to an extensional equality.
`noy-composite` and `yon-composite` specialize to
the noy and yon views.
...
```agda
  ...
  emb-composite-ext f g c =
    emb-ext (emb-composite f g c)
  ...
  unitl f =
    ...
    where
      ...
      lhs = comp idn f (classifier-idn-l f)
          , emb-composite idn f (classifier-idn-l f)
...
```agda
  ...
  E₃-contr f g h cfg cgh .center .snd w' a v b =
    emb-composite (comp f g cfg) h
      (classifier-assoc cfg cgh .fst) w' a v b
    ∙ emb-composite f g cfg w' a v (noy h v b)
...
Every virtual category gives a classified category whose
classifier is `⊤` — every pair composes.

```agda
module _ {o h} (C : V.category o h) where
  private module C = V.Virtual C

  from-virtual : classified o h 0ℓ
  from-virtual .classified.ob = C.ob
  from-virtual .classified.hom = C.hom
  from-virtual .classified.emb = C.emb
  from-virtual .classified.unit = C.unit
  from-virtual .classified.yon-eval = C.yon-eval
  from-virtual .classified.classifier _ _ = ⊤
  from-virtual .classified.classifier-prop = ⊤-is-prop
  from-virtual .classified.compose-classified f g _ =
    C.compose-contr f g
  from-virtual .classified.interchange-classified f g _ =
    C.interchange f g
  from-virtual .classified.classifier-idn-l _ = tt
  from-virtual .classified.classifier-idn-r _ = tt
  from-virtual .classified.classifier-assoc _ _ = tt , tt
```

## To Cat.Virtual

A classified category with a total classifier (every pair is
classified) gives a virtual category: composition and interchange
are available unconditionally.

```agda
module _ {o h p} (C : classified o h p) where
  private module C' = Classified C

  to-virtual
    : (total : ∀ {x y z} (f : C'.hom x y) (g : C'.hom y z)
        → C'.classifier f g)
    → V.category o h
  to-virtual total .V.category.ob = C'.ob
  to-virtual total .V.category.hom = C'.hom
  to-virtual total .V.category.emb = C'.emb
  to-virtual total .V.category.unit = C'.unit
  to-virtual total .V.category.compose-contr f g =
    C'.compose-classified f g (total f g)
  to-virtual total .V.category.interchange f g =
    C'.interchange-classified f g (total f g)
  to-virtual total .V.category.yon-eval = C'.yon-eval
```

### Round-trip

Applying `to-virtual` to `from-virtual V` with the trivial
witness recovers `V`. Each field is definitionally equal, so
the path is constant.

```agda
module _ {o h} (V : V.category o h) where
  private module C = V.Virtual V

  to-virtual-from-virtual
    : to-virtual (from-virtual V) (λ _ _ → tt) ≡ V
  to-virtual-from-virtual i .V.category.ob = C.ob
  to-virtual-from-virtual i .V.category.hom = C.hom
  to-virtual-from-virtual i .V.category.emb = C.emb
  to-virtual-from-virtual i .V.category.unit = C.unit
  to-virtual-from-virtual i .V.category.compose-contr =
    C.compose-contr
  to-virtual-from-virtual i .V.category.interchange =
    C.interchange
  to-virtual-from-virtual i .V.category.yon-eval = C.yon-eval
```
...
chosen. But `interchange` in `V.category` quantifies uniformly
over all contexts — it cannot vary with the classifier value.
Propositionally, `c₁ ≡ c₂` forces the composite to be unique
regardless of witness, which is exactly what `classifier-prop`
guarantees.
