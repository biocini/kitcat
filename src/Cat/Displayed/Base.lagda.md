...
`Cat.Base.theory` clause by clause. Unlike the monoidal `theory₀` —
a transcription whose delta was pure erasure — this is a genuinely
displaced development: each `∙` in a `Cat.Base` proof becomes a
`comp-pathp₁` over the composed base path, each `subst` a transport
along a line of displayed types. The proof shapes follow `Cat.Base`
...
Each link of `Cat.Base`'s unit chain has a displayed image over the
