...
- **`⊗₁-interchange♭-from`** (builder sugar, mirror of
  `⊗₀-interchange♭-from`): the double dependent J over the total
  fibers — each witness's base lines and characterization re-bent
  as a single path in the graph Σ of `⊗₁-composite`
  (`λ i → (p i , p' i) , P i`), one J per side, ambient Σ pinned
  via `J {A = …}` (a pair literal never determines its Σ family).
  The pointwise instance lands definitionally at the refl case.
...
WIP set (open holes, pre-existing). Cold totals over three runs:
`Core.Kan` 0.98 s, `Cat.Monoidal` 1.18 s, `Bifunctor` 1.69 s,
`Cat.Monoidal.Coherence` 6.6 s — all at or under their
pre-session baselines; the coherence module's new definitions
bill 145 ms (`⊗₁-interchange-natural`) + 41 ms (`●₁-coh`), the
pentagon/triangle entries unmoved. Nothing committed.
...
1. `Cat.Monoidal.Indiscrete` port first — the instance that
   builds a `monoidal` from contractible homs; it is where
   `⊗₁-interchange♭-from` earns its keep (pointwise-only builder
   route into the ♭ field at both levels).
