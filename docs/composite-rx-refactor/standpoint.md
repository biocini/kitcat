...
contractibility of a fan; the Rx vocabulary owns that statement. The
identification comes in three tiers of descending definitional
strength, and the tiering is the plan's safety case: the tiers
executed early are conversions the typechecker enforces, and the one
tier that is genuine mathematics is deferred and gated.
...
## Tier 2 — the fibration reading (refl; the load-bearing tier)
...
over `discrete A`, and that is the content of `transp`. This restores
the contractibility half (the reference's `LiftP-is-contr`) that the
in-tree port dropped, and it is the load-bearing tier: after it,
`transp` *means* "discrete families are bifibrations" — the identical
shape, certified in `Test.RxVirtual`, as the frontend's `is-composable`
at the term and coterm displays. `Core.Kan` becomes the worked
Core-level instance of the deductive system's composability tier.

Supporting fact already in-tree: the library's `J` is
`Total-sys-contr .paths` plus a transport (`Core.Transport.J.J-sys`) —
Kan and Transport are one development at the root, and the discipline
...
definitional** — the com-tower and the transp-filler are two centres
...
`HComposite p q r = Σ s , HCell p q r s` has a left leg, a right leg,
a middle, and a composite; `unbiased-lens` has `linj`/`rinj`/
...
The correspondence is real but consists of theorems, not conversions —
the one tier where restatement is mathematics rather than renaming,
and the only tier that would require the lens record below `Core.Kan`
(decision D7). It is deferred to Stage 5; Stage 2 states the reading
in `Core.Kan`'s prose and stops there.

Adjacent fact (Stage 2.4): the three representability developments
are three curryings of one ternary operation — `Core.Groupoid.emb`
(left leg), `Core.Groupoid.Virtual.repr.emb` (middle; refl-equal to
the first — VERIFIED, `Test.KanIdentities` probe 8),
`Core.Path.Composition.Repr` (a third copy of the middle) — with two
full `iso→equiv` proofs (~110 lines) for one operation and a
consumer-less alias `yon-unbiased = repr`. Under ruled D2 they
collapse to the discrete instance of the backend's general form —
the latent virtual-graph theory of the cubical machinery made
explicit ([stage-2-discipline](stage-2-discipline.md)).
`Lens.unb-lens-structure-is-prop` exhibits the general route: uncurry
...
boundary: *the plumbing constructs centres; the Rx layer states what
they are centres of.* Within `Core.Composite` the same boundary falls
between `SysLift`/`syslift-center` (path-shaped lifts: Rx-articulable)
and `SysOver`/`fillerP` (systems with walls over the filler:
plumbing).

## The strata

Reflexive-graph *structure* needs no Kan primitive — pure interval and
Σ (VERIFIED, `Test.RxTier1`: the whole Tier-1 layer compiles on
...
(checked 2026-07-24 in a session probe on the Tier-1 imports plus the
primitive; certificate lands if D6 is pursued) — while the composition
calculus needs `hcom`. Three strata:
...
The module cut currently bundles `transp` and `hcom` in `Core.Kan`, so
both flavors of theory sit above one line; whether the `transp`
stratum is ever reified is decision D6, and no stage depends on the
answer.
