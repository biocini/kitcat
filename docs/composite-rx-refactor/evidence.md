...
Measured 2026-07-24, against the tree at `2f3ac3c` plus the untracked
planning artifacts. Reproduction commands at the end.
...
- Import cost of a Tier-1-sized interface in `Core.Kan` (proxy:
  ...
  **neutral within the noise band**. Caveats: the real `Core.Rx.Base`
  is somewhat larger than the 209-line certificate, and Stage 2's
  restatements are a separate (refl-certified) change.
...
`Core.Path.Exchange` is itself imported only by
`Test.DoubleLoopTensor`. Whether anything survives is decision D9,
not arithmetic.
...
- `Total-sys` — load-bearing through `Total-sys-contr`'s type in
...
- `cone` — zero live consumers, retained as Stage 3.4's transcription
  target (the gist's `ι`).
- `contr-face` — held only by `Core.Coherence.Paths` (red, imported
  only by the retired `src/All.lagda.md`); Stage-5 material, not live
  surface.
...
- The ternary representable action, three curryings
  ...
  (~110 lines) for one operation; `yon-unbiased = repr` an alias with
  no consumers.
...
`Core.Groupoid`'s one live consumer is served and the module joins
`Core.Groupoid.Virtual` at zero live consumers; `Core.Path.Exchange`'s
only importer is `Test.DoubleLoopTensor`. Fates: decisions D9, D10.
...
sweep. Outside the sweep, by hand: `bin/profile` line 12 (usage-string
example), `docs/guidelines/elaboration.md`,
`docs/guidelines/profiling.md`, and the 2026-07-24 notes.

## Provenance of `Core.Composite`

Token-identical to the reference's `Lib.Core.Composite` modulo renames
(`comp`→`com`, `Cyl`→`Sys`, `u`→`s`, `v`→`t`): the bodies of
`fillerP`, `HSysOps`, and `SysFunctor` match λ-for-λ. In-repo history
begins at the file's addition (`a938d00`); the correspondence is
textual. The port dropped `LiftP-is-contr` (restored at Stage 2.1),
`cyl-compose` ([unscheduled](unscheduled.md)), the `Sub`-valued filler
layer (`Fill`, `composite→Ext`, `filler→Fill`), and `subst-cyl`,
