...
`Core.Base`.
`Core.Groupoid` loses its one live consumer with the move
(decision D10 governs the module's fate, at Stage 2.4).
...
sits `private` in `Cat.Graph.Refl.Univalent`, on `--cubical`. It uses
only `Π-is-prop` and `is-contr-is-prop` — nothing from `ua`. It is the
`is-composable` propositionality proof (Stage 3.3/3.5); left where it
is, `deductive-system` would be forced onto full cubical.
...
*Acceptance:* the module's failure mode reduces to its nine holes; the
`Core` count stays 137/139. (The holes are Stage-5 material.)
...
advertises the banned practice. Abstract per the policy (self-contained
fragments with invented names); do not rename through them.
