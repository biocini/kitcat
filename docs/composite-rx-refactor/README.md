...
This directory is the plan of record for the refactor that promotes
the reflexive-graph suite into `Core`, disciplines the Kan machinery
through it from `Core.Composite`'s standpoint, elaborates the
virtual-graph/deductive-system frontend, and rebuilds `Cat`. The
ruled foundation it builds toward is
...
as structure with laws; it is the projected centre of a contractible
...
- **Cubically** (`Core.Kan`, `Core.Composite`, the reference
  `Composite.lagda.md`): a composition problem is a first-class object
  — a system/cylinder with base, walls, lid — and the one theorem is
  that its solution space is contractible. `hcom` picks the centre,
  `hfil` is the contraction, `J` is transport along it, and `transp`
  is the statement that type families lift contractibly.
- **Structurally** (`Cat.Graph.Refl`, promoted to `Core.Rx`; after
  Sterling, *Reflexive Graph Lenses*,
  `resources/sterling-reflexive-graph-lenses`): the vocabulary in
  which the principle is *stated* — fans and cofans with centres,
  univalence as fans-are-propositions, displayed graphs, fibrations as
  contractible lifts, the lens calculus. The backend: nothing anywhere
  in the library re-implements what it encompasses. Ruled (D2), it
  also carries the ternary-action theory — the library develops the
  latent virtual-graph theory of the cubical machinery, part of
  `Core.Composite`'s meaning.
- **Logically** (`Cat.Logic`): a virtual graph is a reflexive graph
  plus `emb`, the ternary representable action; terms and coterms are
  its two representable displays; a deductive system is a virtual
  graph plus one propositional axiom bundle — `is-composable` (both
  fibration conditions at the term and coterm displays: cut
...
The arc, in forced order: promote the backend into `Core` and
discipline the Kan machinery through it (Stages 0–2); build the
frontend (Stage 3); rebuild `Cat` on the deductive system, with
`Cat.Depreciated` as the porting reference until then (Stage 4); only
then refound the Core coherence work (Stage 5) — in the
deductive-system shape, with the path groupoid as the canonical
discrete instance (`pcom` as its `emb`) and `Core.Kan` as the worked
example of the composability tier. The telos is the LB certification
program (`resources/mellies-ribbon-tensorial-logic`,
...
- **Frontend/backend compatibility** (Lane, 2026-07-24). A virtual
  graph is a reflexive graph with extra structure, so the logic layer's
  constructions elaborate to their `Core.Rx` counterparts on the nose.
  The frontend therefore names and defines freely in domain
  vocabulary; what it does not do is re-derive theory the backend
  already proves, since every `Core.Rx` lemma applies to a virtual
  graph without a bridge. Definitions must land definitionally —
  agreement only up to a path would force exactly the bridge the
  layering exists to avoid. The discipline also runs in reverse: the
  backend may speak the virtual-graph language before the theory is
  formally introduced, since an implementation already shaped by the
  metatheory makes its eventual conformance trivial to exhibit.
- **Cycle-driven stratification** (Lane, 2026-07-24). Machinery
  offloads from `Core.Rx` into Properties-style modules *when cyclic
  imports force it*, not by a-priori topic. Placement above `Core.Kan`
  requires a forcing cycle; [architecture](architecture.md) records
  the forced set and the unforced remainder (decision D7).
- **Bundled virtual graph** (Lane, 2026-07-24). `virtual-graph`
  contains its reflexive graph as a field and re-exports the domain
  names (`ob`, `hom`, `term`, `coterm`, …); the inference cost is
  measured (`Test.RxBundle`) and mitigated by keying the interface off
  a module parametrized by the structure.
...
- **The ternary action descends** (Lane, 2026-07-24; decision D2,
  ruled). The backend carries the virtual-graph theory: the latent
  virtual-graph theory of the cubical machinery is developed
...
projections; never bridge two constructions), generic lemmas extracted
...
`Core.Path.Composition` 512, `Core.Composite` 161, plus dead surface).
Under the plan it lands near ≈ 1,600: dead surface −120, the
displaced-composition family −430 at Stage 4, the alias collapse, and
the emb deduplication with three module retirements. Elaboration is
neutral through Stages 0–2 (measured — [evidence](evidence.md)) and
≈ 300 ms cheaper after Stage 4. The statement the smaller Core makes
is the program's: `transp` exhibited as the fibration witness over
`discrete A` — the same certified shape as the frontend's
`is-composable` — so Stage 5's coherence work and the frontend's
associahedron towers share one contractibility engine.
