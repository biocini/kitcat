...
`Core.Kan` and `Core.Composite` restated in the backend's
vocabulary, per the tiers of the [standpoint](standpoint.md), with
the dead surface and the duplications retired. Everything here is
Tier 1–2 material (conversions and certified identities) plus
deletions licensed by the [evidence](evidence.md) censuses.
...
**The canonical-construction ruling, written in.** The
identification `SysP.fillerP ~ cov-fibration.lift` is propositional,
not definitional: a com-tower and a transp-filler are two centers of
one contractible space. Per the one-construction principle and the
fiber-projection idiom (never declare the structure; declare a
contractible fiber and project):

- `SysLift`/`syslift-center` become fibration projections. The
  com-based construction retires from that role.
- **State no bridging lemma** between the two constructions.
- `SysOver`/`fillerP` (systems *with walls* over the filler,
  strictly more than the path case) remain plumbing, per the Exo
  boundary.

## 2.2 Fan vocabulary in `Core.Kan`

- `Total-sys φ s` restated as
  `rx.fan (discrete A) (sys-composite φ s)`: refl
  (`Test.KanIdentities` probe 3), so consumers
  (`Core.Transport.J.J-sys`) stay untouched.
...
- The module's opening prose states the plumbing/statement boundary
  (*the plumbing constructs centers. The Rx layer states what they
  are centers of*) and the Tier-3 reading of `HComposite`. Prose
  only. The lens restatement is Stage 5 material, gated on D7.

## 2.3 Dead surface, aliases, relocations

Delete (zero consumers src-wide, ≈ 120 lines + `hcom-unique`'s
47 ms, full list with liveness notes in [evidence](evidence.md)):
`HSys`, `sys-path`, `SysExt`, `sys-lid`, `hc`, `kext`, `TotalP`,
...
speaks `hcom`, 2.2's boundary), `Sys`/`PartialsP` to one name
(`Sys φ A` is `PartialsP φ (λ _ → A)`, VERIFIED), and `sys-filler`
against `hfil` (argument permutation). Re-point `Core.Composite`
and `Core.Transport.J`.

Relocate `Chain` to `Core.Path`, updating `Core.Univalence`'s import
(the one live consumer of the combinators).

Keep, with their reasons on record: `Total-sys` (in use through
...
Delete `comp-pathp` (zero consumers anywhere). The other nineteen
...
**The placement question.** Each held name carries one open
question: *does the construction have a theoretical placement in the
...
and its consequences over a composite), or is it an artifact of its
ad hoc construction route?

**Disposition, per name, before Stage 4 closes.** Exactly two
outcomes:

- *Placement found*: the construction gets its reformulation over
  the disciplined structure, in its principled home, and the
  displaced form retires. The name need not survive. The mathematics
  does.
- *No placement*: it was the deprecated tree's scaffolding, and it
  leaves with that tree.

There is no third outcome. No name enters the rebuilt `Cat` on the
grounds that it already exists, and the disciplined structure does
not bend to receive one.

**Timing.** The question becomes answerable when 2.1 lands and
sharpens at Stage 5, when the target shape settles. Answer it for
every held name before Stage 4 deletes the deprecated tree: that is
the last moment a consumer exists to check a reformulation against.
Until then the held names take no new consumers.
