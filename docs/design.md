...
The presentation is a trilayer. The operations (including the two
actions `pre`/`post`) and every axiom-free derived notion live in
`codep-structure`; the five axioms and the extraction live in
`codep-axioms`, stated over a structure value; the universe-ranging
`codep-category` bundles the two:

```
record codep-structure (ob : Type o)         -- hom/idn/emb, pre/post + derived
record codep-axioms (S : codep-structure ob) -- 5 axioms + _⨾_ + laws
record codep-category (o h : Level)           -- fields ob, structure, axioms
```

Because the axioms record is complete, the bundle IS the category and
the refactor equation is literal: `Cat.Type.category ≅ codep-category`.
`compose-contr` is the one axiom from which composition, associativity,
and the pentagon derive; the other four (`interchange`, `post-eval`,
`unit-eqvl`, `unit-eqvr`) pin the anchor as a genuine unit.

Splitting the axioms off the operations makes naive multi-object
instances termination-safe: `codep-axioms` states `compose-contr` over
an *external* structure value, so a direct instance's proof only
projects that closed value — a `hom`/`idn` that cases on the object no
longer leaves a stuck `idn y` self-call in the proof term.
...
(dep₂)); `compose-contr` is the composition axiom.
...
asserted or used. The two instances make this concrete: `type-instance`
fills `idn` with an identity morphism (`C.idn`), `monoidal-instance`
fills it with the tensor unit *object* `I`; the shared role is "anchor", not
"two-sided unit". This is the pre-side structure through the pentagon.

### The identity fragment: two layers over the anchor

The scope note above is resolved by the four anchor axioms now merged
into `codep-axioms`, with the derived-law modules `Cat.Codep.Coupling`
(`coupling-laws`) and `Cat.Codep.Unit` (`unit-laws`) characterizing the
anchor. Both derived-law modules gate uniformly on the bundle
`(C : codep-category o h)`.

The **coupling** axioms relate the two representable actions — `pre`
(acting on the acted slot) and `post` (acting on the passenger binder),
themselves structure-level. They are `interchange` (the actions
commute; this is the L3.5 profunctorial link) and `post-eval` (the
passenger action at the identity is the identity). From the coupling
*alone* the identity idempotency `idem : idn ⨾ idn ≡ idn` derives, via
`post-comp` + `comp-eq`. This is the **linchpin**: idempotency precedes
absorption, and the two are not circular. The guarantee is a theorem —
the lemma `idem-from-coupling`, whose hypotheses are exactly
`compose-contr`, `interchange`, `post-eval` (and nothing else). Its
typechecked signature *is* the absorption-freeness fact: it cannot
name `unit-eqvl`/`unit-eqvr` or `absorb`, so non-usage is machine-
checked. (This replaced an earlier module-boundary formulation of the
same guarantee.)

The **unit** axioms are invertibility of the identity's two actions
(`unit-eqvl`, `unit-eqvr`). From these, absorption
...
With the coupling and unit axioms merged into `codep-axioms`, the
refactor blueprint is now literal: `Cat.Type.category ≅ codep-category`
— the four-axiom canonical record IS the bundle, decomposed internally
into the representability axiom plus the coupling and unit
characterizations of its anchor. The same holds over `Cat.Monoidal`
(the anchor being the tensor unit `I`), where the generic `absorb-l`
coincides with the concrete one definitionally, and
`unitl`/`unit-is-prop` specialize in statement (two fibers giving the
same edge).
