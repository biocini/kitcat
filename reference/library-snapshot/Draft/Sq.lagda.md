...
open import Core.Kan using (transp; hcom; hfill; coe0i; coe01)
...
module _ {u} {A : Type u} where
  ...
  lunit {x} p i j = hcom (∂ j ∨ ~ i) λ where
    k (j = i0) → x
    k (i = i0) → p (j ∧ k)
    k (j = i1) → p k
    k (k = i0) → x
  ...
  assoc {w} {z = z} p q r i j = hcom (∂ j ∨ i) λ where
    k (j = i0) → w
    k (i = i1) → concat.fill (concat p q) r j k
    k (k = i0) → concat.fill p q j i
    k (j = i1) → hcom (∂ k ∨ i) λ where
      l (i = i1) → r (k ∧ l)
      l (k = i0) → q i
      l (k = i1) → r l
      l (l = i0) → q (k ∨ i)

{-# DISPLAY hcom _ (concat.sys p q _) = concat p q #-}
