...
-- The associativity data needed to show neutral morphisms are composable:
-- g linear at f, and f thunkable at g.
composable : ∀ {a b c} (f : hom a b) (g : hom b c) → Type (o ⊔ h)
composable f g = linear g f × thunkable f g
...
module is-neutral {x y} {f : hom x y} ((ld , rd) : is-neutral f) where
  ...
  is-thunkable→loop-unitl : is-thunkable f → ∀ {z} (g : hom y z) → loop ⨾ g ≡ g
  is-thunkable→loop-unitl et g = divl→lcancel ld (et loop g ∙ pre-counit f ▹ g)

  is-linear→coloop-unitr : is-linear f → ∀ {w} (h : hom w x) → h ⨾ coloop ≡ h
  is-linear→coloop-unitr el h = divr→rcancel rd (sym (el h coloop) ∙ h ◃ post-counit f)
