...
equivalence rather than merely an embedding. The identity morphism,
composition, and all categorical laws are derived from the equivalence
inverse and counit.
...
Street and Walters (1978) axiomatized the Yoneda embedding as
primitive data, but at the 2-categorical level — their axioms
describe how presheaf constructions behave across a 2-category of
categories, presupposing that the objects already have internal
categorical structure. Riehl and Shulman (2017) and Capriotti and
Kraus (2018) define (∞-)categories by requiring the space of
composites to be contractible (the Segal condition), which is close
in spirit but requires a separate completeness condition for
identities.

This formulation combines both ideas at the level of a single
category: a single equivalence condition on the representability map
`yon : hom x y → (∀ w → hom w x → hom w y)` subsumes the Segal
condition (contractible composite fibers fall out of `is-equiv`)
and recovers identities (as `yon⁻¹ (λ _ → id)`), with no additional
axioms. The standard HoTT formalization libraries (1lab, UniMath,
agda-unimath) all take the algebraic approach — giving identity,
composition, and laws as separate fields, then proving Yoneda as a
theorem.
...
```agda
    ...
    -- Two paths with endpoints in the same composite fiber are equal.
    -- Both sides lift to the contractible (hence set) is-composable fiber,
    -- and ap fst of each lift recovers the original path definitionally.
    composite-path-unique
      : ∀ {x y z} (f : hom x y) (g : hom y z)
        {a b : hom x z}
        (α : composite f g a) (β : composite f g b)
        (p q : a ≡ b)
        (αp : PathP (λ i → composite f g (p i)) α β)
        (αq : PathP (λ i → composite f g (q i)) α β)
      → p ≡ q
    composite-path-unique f g α β p q αp αq =
      total-contr-unique (composite-contr f g) p q αp αq
