...
is faithfully represented by the function
  `(_ ⨾ f) : hom w x → hom w y`.
...
record category o h : Type₊ (o ⊔ h) where
  ...
  field
    emb : ∀ {x y} → is-embedding (yon {x} {y})
    emb-op : ∀ {x y} → is-embedding (noy {x} {y})
    unital : ∀ {x} → fiber (yon {x} {x}) (λ _ → id)
    unital-op : ∀ {x} → fiber (noy {x} {x}) (λ _ → id)
    yon-path : ∀ {x y z} {f : hom x y} {g : hom y z} → yon (f ⨾ g) ≡ (λ w → yon g w ∘ yon f w)
    noy-path : ∀ {x y z} {f : hom x y} {g : hom y z} → noy (f ⨾ g) ≡ (λ a → noy f a ∘ noy g a)

  comp : ∀ {x y z} {f : hom x y} {g : hom y z} → fiber (yon {x} {z}) (λ w → yon g w ∘ yon f w)
  comp {f} {g} = (f ⨾ g) , yon-path
