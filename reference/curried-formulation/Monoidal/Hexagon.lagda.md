...
(`Cat.Depreciated.Monoidal.Braid`), but the hexagon is not: it relates the
...
in `Cat.Depreciated.Monoidal.Coherence`. Six vertices sit in a single fiber
...
module Cat.Depreciated.Monoidal.Hexagon where
...
open import Cat.Depreciated.Monoidal.Braid
```
