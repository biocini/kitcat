...
presentation of categories in `Cat.Depreciated.Type`, but stated directly
...
contractible composition fibers exactly as `Cat.Depreciated.Type` derives
`assoc`, `unitl`, and `unitr`.

The correspondence with `Cat.Depreciated.Type` erases the two `Unit`
...
to a later `Cat.Depreciated.Monoidal.Coherence` submodule.
...
— mirroring the base `Cat.Depreciated.Type._·_`; the tensor composite
...
- **Pentagon and triangle**: to be derived natively for the
  tensor in a new `Cat.Depreciated.Monoidal.Coherence` submodule.
