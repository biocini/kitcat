...
`Cat.Depreciated.Coherence`'s old record-level weak triangle onto the flat
carrier, reusing `assoc-tower` (from `Cat.Depreciated.Codep.Coherence`) for
...
(`face₂₃`, the paid unitl face). It closes against the `Cat.Depreciated.Codep`
stack once the coherence overlay `A2 : category-2-coherent C` is in
scope — it needs no fourth cell. The mechanism:

  * The library `unitl g` (`Cat.Depreciated.Type`) is `ap fst` of a path in
  ...
  * The overlay `absorb-lcoh` supplies `absorb-l` in the `post-eval`
    ...
    field: it is `gauge-r` (`Cat.Depreciated.Codep.Coherent`), the homotopy-
...
a misattribution. The `Cat.Depreciated.Codep.Coherent` header's S²/π₃ independence
...
module Cat.Depreciated.Codep.Triangle where
...
open import Cat.Depreciated.Codep.Coherence using (module assoc-tower)
open import Cat.Depreciated.Codep.Coherent using (category-2-coherent; op-coherent)
open import Cat.Depreciated.Codep.Op using (op)
```
...
triangle follows. The bridge is `gauge-r` (from `Cat.Depreciated.Codep.Coherent`):
