...
Cat.Depreciated.Type.category is the special case where every pair
composes (classifier is trivially ⊤).

```agda
{-# OPTIONS --safe --erased-cubical --no-guardedness --no-sized-types #-}

module Cat.Depreciated.Virtual where
...
import Cat.Depreciated.Type as T
```
