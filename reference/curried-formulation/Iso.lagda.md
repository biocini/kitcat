...
module Cat.Depreciated.Iso where
...
open import Cat.Depreciated.Base using (module Cat)
open import Cat.Depreciated.Type using (category)
...
```agda
  -- Following Rijke Ch 5 (J); cf. 1lab Cat.Depreciated.Univalent path→iso
  idtoiso : ∀ {x y} → x ≡ y → x ≅ y
...
```agda
  hom-PathP≡Square
    : ∀ {x x' y y'} (p : x ≡ x') (q : y ≡ y')
    → (f : hom x y) (g : hom x' y')
    → (PathP (λ i → hom (p i) (q i)) f g) ≡ (f ⨾ idtoiso q .fst ≡ idtoiso p .fst ⨾ g)
...
It's a nice result demonstrating the naturality square of idtoiso.
