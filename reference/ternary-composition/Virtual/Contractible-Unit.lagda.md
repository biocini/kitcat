...
```agda
  ...
  unitl f =
    ap fst (is-contr→is-prop (composable-contr idn f) lhs rhs)
    where
      lhs : fiber emb
        (λ w a v b → emb idn w a v (noy f v b))
      ...
      rhs : fiber emb
        (λ w a v b → emb idn w a v (noy f v b))
      rhs = f , funext λ w → funext λ a → funext λ v →
...
```agda
  ...
  module triangle-fibers
    ...
    private
        ...
        : (   f ⨾ idn
            ...
            ∙ funext λ w → funext λ a →
              funext λ v → funext λ b →
                ap (emb f w a v) (absorb-l b))
        ≡ (f , refl)
      unitr-σ = is-contr→is-prop (emb-image-contr f) _ _
      ...
      -- Full sigma path underlying assoc f idn g, living
      -- in E₃-contr f idn g.
      assoc-σ-fig
        : (   (f ⨾ idn) ⨾ g
            , emb-composite (f ⨾ idn) g
      ...
      -- At i=1: same reassociation in the other direction.
      w₂ : γ₁₂-pt i1 ≡ pt₂
      w₂ i =
        f ⨾ (idn ⨾ g)
        , sym (Path.assoc
...
For the triangle, `face₁₃` lifts `unitr` from
`emb-image-contr f` to `composable-contr f g` with a
`Path.unitr` correction at i=1. `face₁₂` lifts `assoc`
from `E₃-contr f idn g` with `Path.assoc` corrections at
both boundaries (the same pattern as the pentagon's
un-whiskered edges). The classical triangle follows from
`hom-identity ≡ face₁₃⁻¹ ∙ face₁₂`.
