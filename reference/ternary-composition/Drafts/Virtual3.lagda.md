...
axiom and absorption coherence. The `compose-contr` field bundles
the composite morphism, its noy-characterization, and its
yon-characterization into a single contractible type. This replaces
the earlier `composable-contr` + `interchange` pair, giving both
characterizations in one shot. The `absorb-coh` field is retained
for the Mac Lane triangle identity.
...
so that `absorb-coh` can reference them. The `compose-contr` field
bundles the composite, its noy-characterization, and its
...
record category o h : Type₊ (o ⊔ h) where
  ...
  -- Derived from compose-contr: go through f ⨾ g.
  interchange
    : ∀ {x y z} (f : hom x y) (g : hom y z)
      w (a : hom w x) v (b : hom z v)
    → emb f w a v (noy g v b)
...
module Cat {o} {h} (C : category o h) where
  ...
  -- Derived from compose-contr: project out the
  -- noy-characterization.
  composable-contr
    : ∀ {x y z} (f : hom x y) (g : hom y z)
    → is-contr
        (fiber (emb {x} {z})
